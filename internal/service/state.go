...
func GetUserState(ctx context.Context, db *sql.DB, chatID int64) string {
	var state string
	query := `SELECT current_state FROM users WHERE chat_id = $1`

	err := db.QueryRowContext(ctx, query, chatID).Scan(&state)
	if err != nil {
...
func SetUserState(ctx context.Context, db *sql.DB, chatID int64, newState string) error {
	query := `
    ...
    `
	_, err := db.ExecContext(ctx, query, chatID, newState)
	return err
}
